...
scriptName <- "XgBoostPerStoreWithSalesFeatures1"
set.seed(13)
os <- Sys.info()[["sysname"]]
...
train <- merge(train[train$StoreType!='c',],salesDistributionPerStore,by=c("Store","Month","DayOfWeek","HalfOfMonth"))
...
test1 <- merge(test[test$StoreType!='c',],salesDistributionPerStore,by=c("Store","Month","DayOfWeek","HalfOfMonth"),all.x=T)
...
submission <- rbind(submission,predictPerStoreType(testC))
