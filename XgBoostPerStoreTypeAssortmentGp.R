...
startTime <- Sys.time()
factorsAsIntegers <- F
scriptName<-ifelse(factorsAsIntegers,"XgBoostPerStoreTypeAssortmentGpFactorsAsIntegers","XgBoostPerStoreTypeAssortmentGp")
set.seed(13)
os <- Sys.info()[["sysname"]]
...
groupsOrderInTrain <- character()
trainPerGroup <- function(df){
  set.seed(13)
  print(paste("nrow",nrow(df),"store type",unique(df$StoreType)))
  h<-sample(nrow(df),0.2*nrow(df))
  gp <- as.character(unique(df$Group))
  samples[[gp]] <<- h
  groupsOrderInTrain <<- c(groupsOrderInTrain,gp)
...
train$Group <- paste(train$StoreType,train$Assortment,sep="_")
test$Group <- paste(test$StoreType,test$Assortment,sep="_")
dataPerGroup<- lapply(unique(train$Group),function(x){train[train$Group==x,]})
dataPerGroupNames <- unlist(lapply(dataPerGroup,function(x){unique(x$Group)}))
models <- lapply(dataPerGroup,trainPerGroup)
names(models) <- dataPerGroupNames
names(dataPerGroup) <- dataPerGroupNames
predictPerGroup <- function(df){
  Id=df$Id
  gp <- unique(df$Group)
  t <- df[,feature.names]
  return(data.frame(Id, Sales=exp(predict(models[[gp]],data.matrix(t)))-1))
}
dataPerGroupTest<- lapply(unique(test$Group),function(x){test[test$Group==x,]})
submission <- do.call(rbind,lapply(dataPerGroupTest,predictPerGroup))
...
print(mean(unlist(lapply(models,function(x){x$bestScore}))))
