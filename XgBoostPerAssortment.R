...
startTime <- Sys.time()
factorsAsIntegers <- F
scriptName<-ifelse(factorsAsIntegers,"XgBoostPerAssortmentFactorsAsIntegers","XgBoostPerAssortment")
set.seed(13)
os <- Sys.info()[["sysname"]]
...
trainPerStoreType <- function(df){
  set.seed(13)
  print(paste("nrow",nrow(df),"store type",unique(df$Assortment)))
  h<-sample(nrow(df),0.2*nrow(df))
  samples[[as.character(unique(df$Assortment))]] <<- h
  storeTypesOrderInTrain <<- c(storeTypesOrderInTrain,as.character(unique(df$Assortment)))
...
dataPerStoreType<- lapply(unique(train$Assortment),function(x){train[train$Assortment==x,]})
dataPerStoreTypeNames <- unlist(lapply(dataPerStoreType,function(x){unique(x$Assortment)}))
...
predictPerStoreType <- function(df){
  Id=df$Id
  t <- df[,feature.names]
  return(data.frame(Id, Sales=exp(predict(models[[as.character(unique(t$Assortment))]],data.matrix(t)))-1))
}
dataPerStoreTypeTest<- lapply(unique(test$Assortment),function(x){test[test$Assortment==x,]})
